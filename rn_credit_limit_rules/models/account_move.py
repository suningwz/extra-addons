...
class AccountMove(models.Model):
    ...
    def action_post(self):
        if self.partner_id.credit_limit_rules_id and self.move_type in ['out_invoice'] and not self.allow_credit:
            invoices = self.env['account.move'].sudo().search([('move_type', 'in', ['out_invoice', 'out_refund']),
                                                               ('amount_residual_signed', '<>', 0.0),
                                                               ('state', '=', 'posted')])
            ...
            if self.partner_id.credit_limit_rules_id.type == 'documents':
                ...
                if len(invoices) > minimo_doc:
                    raise ValidationError(_('The partner has a credit limit rule based '
                                            'on the number of unpaid documents!'))
    ...
    def action_get_credit_rule_summary(self):
        ...
        ctx = dict(default_partner_id=self.partner_id.id, default_account_move_id=self.id)
